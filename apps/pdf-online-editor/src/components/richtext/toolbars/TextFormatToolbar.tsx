...
export default function TextFormatToolbar({
  ...
  const ColorPicker = ({ onSelect, currentColor }: { onSelect: (color: string) => void; currentColor: string }) => (
    <div className="absolute z-10 bg-white border border-slate-200 rounded-lg shadow-lg p-3 mt-1">
        ...
        {COMMON_COLORS.map((color) => (
          <button
            ...
            className={`w-6 h-6 rounded border-2 ${
              currentColor === color ? 'border-slate-900' : 'border-slate-200'
            }`}
  ...
  return (
    <div className="flex items-center space-x-2 px-3 py-2 bg-slate-50 border-b border-slate-200">
      ...
      <select
        ...
        disabled={disabled}
        className="px-2 py-1 border border-slate-200 rounded text-sm bg-white disabled:opacity-50 cursor-pointer"
      >
      ...
      <div className="relative">
        <button
          ...
          disabled={disabled}
          className="px-2 py-1 rounded text-sm border border-slate-200 bg-white hover:bg-slate-100 transition-colors disabled:opacity-50 flex items-center space-x-1"
          title="Text Color"
        >
          <span>A</span>
          <div
            className="w-4 h-1 border border-slate-200"
